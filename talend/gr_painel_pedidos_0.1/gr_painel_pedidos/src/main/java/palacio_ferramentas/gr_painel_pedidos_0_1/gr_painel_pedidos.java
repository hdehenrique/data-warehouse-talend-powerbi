...
+"ALIB) IS NOT NULL THEN 'AG. SEPARACAO'\n    WHEN ADTPED.VALOR >= F.FGO_VLRCON THEN 'AG. LIBERACAO'\n    WHEN ADTPED.VALO"
+"R < F.FGO_VLRCON AND ADTPED.VALOR > 0 THEN 'AG. LIBERACAO (PGTO PARCIAL)'\n    WHEN ADTPED.VALOR = 0 OR ADTPED.VALOR IS "
+"NULL THEN 'AG. PAGAMENTO'\n    ELSE 'OUTROS?'\n  END AS STATUS\n  FROM FATGOR F\n  INNER JOIN EXTGOR E ON F.AUTOINCREM ="
+" E.FGO_SEQFGO\n  LEFT JOIN MOVGER M ON\n    F.FGO_NUMFAT = M.GER_NUMDOC\n    AND F.FGO_TIPFAT = M.GER_ESPDOC\n    AND F."
+"FGO_EMPFAT = M.GER_CODEMP\n    AND F.FGO_CODCLI = M.GER_CODCLI\n  LEFT JOIN MOVDET DV ON M.GER_NUMDOC = DV.DET_DOCDEV AN"
+"D DV.DET_ESPDEV = M.GER_ESPDOC AND DV.DET_EMPDEV = M.GER_CODEMP\n  INNER JOIN CADCLI AS C ON F.FGO_CODCLI = C.CLI_CODCLI"
+"\n  INNER JOIN CADOPE AS V ON F.FGO_CODVEN = V.OPE_CODOPE\n  LEFT JOIN CADOPE AS S ON E.FGO_SEPEXP = S.OPE_CODOPE\n  LEF"
+"T JOIN (\n  SELECT\n    FG.AUTOINCREM FG_AUTOINCREM, AD.ADT_NUMDOC DOCUMENTO, AD.ADT_ESPDOC ESPECIE, AD.ADT_CODEMP EMPRE"
+"SA, -- MAX(CONCAT(AD.ADT_DTALAN, ' ', AD.ADT_HORLAN)) DTALAN,\n    MAX(CONCAT(DATE_FORMAT(AD.ADT_DTALAN , '%d/%m/%Y'), '"
+" ', DATE_FORMAT(AD.ADT_HORLAN, '%H:%i'))) DTALAN,\n    SUM(IF(AD.ADT_TIPLAN = 'D', AD.ADT_VALLAN * (-1), AD.ADT_VALLAN))"
+" VALOR, COUNT(*) QT\n  FROM ADTCLI AD\n    INNER JOIN FATGOR FG ON FG.FGO_NUMDOC = AD.ADT_NUMDOC AND FG.FGO_ESPDOC = AD."
+"ADT_ESPDOC AND FG.FGO_CODEMP = AD.ADT_CODEMP\n    INNER JOIN EXTGOR EG ON FG.AUTOINCREM = EG.FGO_SEQFGO\n  WHERE TRUE\n "
+" AND EG.FGO_DH_IMP > DATE_SUB(CURDATE(), INTERVAL 60 DAY)\n  AND ADT_CODEMP = '001'\n  AND AD.ADT_DESCRI NOT LIKE 'DEBIT"
+"O REF. EMISS%'\n  GROUP BY DOCUMENTO, ESPECIE, EMPRESA) AS ADTPED ON F.AUTOINCREM = ADTPED.FG_AUTOINCREM\n  WHERE TRUE\n"
+"  AND F.FGO_MOTBAI IS NULL\n  AND F.FGO_CODEMP = '001'\n  AND F.FGO_ESPDOC = 'PD'\n  AND M.GER_NUMROM IS NULL\n  AND DV."
+"AUTOINCREM IS NULL\n  AND F.FGO_DTAENT > DATE_SUB(CURDATE(), INTERVAL 60 DAY)\n  AND F.FGO_CODTRA IS NOT NULL\n  AND V.O"
+"PE_CODDEP IN ('014', '002')  \n  ) AS TB\n  INNER JOIN CADTRA CT ON TB.FGO_CODTRA = CT.TRA_CODTRA \n  GROUP BY FGO_CODTR"
+"A\nUNION ALL  \n  SELECT 'VENDEDOR' AS ID,\n  FGO_CODVEN AS CODIGO,\n  CO.OPE_NOMOPE AS NOME,\n  SUM(IF (STATUS='AG. PAG"
+"AMENTO', 1, 0)) AS 'AG_PGTO',\n  SUM(IF (STATUS='AG. LIBERACAO', 1, 0)) AS 'AG_LIBERACAO',\n  SUM(IF (STATUS='AG. SEPARA"
+"CAO', 1, 0)) AS 'AG_SEPARACAO',\n  SUM(IF (STATUS='EM SEPARACAO', 1, 0)) AS 'EM_SEPARACAO',\n  SUM(IF (STATUS='AG.EXPEDI"
+"CAO', 1, 0)) AS 'AG_EXPEDICAO',\n  SUM(IF (STATUS IN ('FATURADO/AG. ROMANEIO', 'EXPEDIDO/AG. ROMANEIO'), 1, 0)) AS 'AG_R"
+"OMANEIO'\n  FROM\n  (\n  select F.FGO_CODVEN, F.FGO_NUMDOC, ADTPED.VALOR,\n  CASE\n    WHEN DV.AUTOINCREM IS NOT NULL TH"
+"EN 'DEVOLVIDA'\n    WHEN M.GER_NUMROM IS NOT NULL THEN 'ENVIADO'\n    WHEN M.GER_NUMDOC IS NOT NULL THEN 'FATURADO/AG. R"
+"OMANEIO'\n    WHEN E.FGO_OPEEXP IS NOT NULL THEN 'EXPEDIDO/AG. ROMANEIO'\n    WHEN E.FGO_SEPEXP IN ('599','804','877','8"
+"80','881') THEN 'EXCECOES'\n    WHEN IFNULL(E.FGO_OPBXSE, E.FGO_SEPEXP) IS NOT NULL THEN 'AG.EXPEDICAO'\n    WHEN IFNULL"
+"(E.FGO_DTIMSE, E.FGO_ONDADT) IS NOT NULL THEN 'EM SEPARACAO'\n    WHEN F.FGO_DTALIB IS NOT NULL THEN 'AG. SEPARACAO'\n  "
+"  WHEN ADTPED.VALOR >= F.FGO_VLRCON THEN 'AG. LIBERACAO'\n    WHEN ADTPED.VALOR < F.FGO_VLRCON AND ADTPED.VALOR > 0 THEN"
+" 'AG. LIBERACAO (PGTO PARCIAL)'\n    WHEN ADTPED.VALOR = 0 OR ADTPED.VALOR IS NULL THEN 'AG. PAGAMENTO'\n    ELSE 'OUTRO"
+"S?'\n  END AS STATUS\n  FROM FATGOR F\n  INNER JOIN EXTGOR E ON F.AUTOINCREM = E.FGO_SEQFGO\n  LEFT JOIN MOVGER M ON\n  "
+"  F.FGO_NUMFAT = M.GER_NUMDOC\n    AND F.FGO_TIPFAT = M.GER_ESPDOC\n    AND F.FGO_EMPFAT = M.GER_CODEMP\n    AND F.FGO_C"
+"ODCLI = M.GER_CODCLI\n  LEFT JOIN MOVDET DV ON M.GER_NUMDOC = DV.DET_DOCDEV AND DV.DET_ESPDEV = M.GER_ESPDOC AND DV.DET_"
+"EMPDEV = M.GER_CODEMP\n  INNER JOIN CADCLI AS C ON F.FGO_CODCLI = C.CLI_CODCLI\n    INNER JOIN CADOPE AS V ON F.FGO_CODV"
+"EN = V.OPE_CODOPE\n  LEFT JOIN CADOPE AS S ON E.FGO_SEPEXP = S.OPE_CODOPE\n  LEFT JOIN (\n  SELECT\n    FG.AUTOINCREM FG"
+"_AUTOINCREM, AD.ADT_NUMDOC DOCUMENTO, AD.ADT_ESPDOC ESPECIE, AD.ADT_CODEMP EMPRESA, -- MAX(CONCAT(AD.ADT_DTALAN, ' ', AD"
+".ADT_HORLAN)) DTALAN,\n    MAX(CONCAT(DATE_FORMAT(AD.ADT_DTALAN , '%d/%m/%Y'), ' ', DATE_FORMAT(AD.ADT_HORLAN, '%H:%i'))"
+") DTALAN,\n    SUM(IF(AD.ADT_TIPLAN = 'D', AD.ADT_VALLAN * (-1), AD.ADT_VALLAN)) VALOR, COUNT(*) QT\n  FROM ADTCLI AD\n "
+"   INNER JOIN FATGOR FG ON FG.FGO_NUMDOC = AD.ADT_NUMDOC AND FG.FGO_ESPDOC = AD.ADT_ESPDOC AND FG.FGO_CODEMP = AD.ADT_CO"
+"DEMP\n    INNER JOIN EXTGOR EG ON FG.AUTOINCREM = EG.FGO_SEQFGO\n  WHERE TRUE\n  AND EG.FGO_DH_IMP > DATE_SUB(CURDATE(),"
+" INTERVAL 60 DAY)\n  AND ADT_CODEMP = '001'\n  AND AD.ADT_DESCRI NOT LIKE 'DEBITO REF. EMISS%'\n  GROUP BY DOCUMENTO, ES"
+"PECIE, EMPRESA) AS ADTPED ON F.AUTOINCREM = ADTPED.FG_AUTOINCREM\n  WHERE TRUE\n  AND F.FGO_MOTBAI IS NULL\n  AND F.FGO_"
+"CODEMP = '001'\n  AND F.FGO_ESPDOC = 'PD'\n  AND M.GER_NUMROM IS NULL\n  AND DV.AUTOINCREM IS NULL\n  AND F.FGO_DTAENT >"
+" DATE_SUB(CURDATE(), INTERVAL 60 DAY)  ) AS TB\n  INNER JOIN CADOPE CO ON TB.FGO_CODVEN = CO.OPE_CODOPE\n  GROUP BY FGO_"
+"CODVEN\n";
...
/************************************************************************************************
 *     266125 characters generated by Talend Open Studio for Big Data 
 *     on the 24 de junho de 2024 10:59:21 BRT
 ************************************************************************************************/